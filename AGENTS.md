...
El nuevo módulo Equipo EVO no debe romper ni redefinir esos modos.
...
Para la Fase 1 visual, no deben necesitarse nuevas variables ni conexiones reales.
...
No debe convertirse en propietario único de:
...
La Fase 1 es solo visual y local.
...
El repositorio contiene lógica específica para evitar clientes con bundles obsoletos y usa service worker en producción.

No modificar:

- protección de build id;
- service worker;
- política de caché;
- `vite.config.js`;
- `vercel.json`;

salvo que la fase lo requiera explícitamente y exista una prueba de regresión.
...
- Documentación viva creada en `docs/evo-live-docs-v1`.
- Código modificado: no.
- Migraciones modificadas: no.
- Producción modificada: no.
- Fase activa: ninguna.
- Siguiente fase propuesta: Fase 1 visual y local.
