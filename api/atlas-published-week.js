...
async function loadPublishedWeek({ supabase, weekStartDate }) {
  const select = 'week_start_date, mesociclo, semana, titulo, published_at, publication_status, data'
  let result = await supabase.from('published_weeks')
    .select(select)
  ...
  if (result.error && isMissingPublicationStatusError(result.error)) {
    result = await supabase.from('published_weeks')
...
export function createAtlasPublishedWeek({
  ...
  return async (req, res) => {
    ...
    if (!isMondayIsoDate(weekStartDate)) {
    ...
    try {
      ...
      const week = await load({ supabase, weekStartDate })
