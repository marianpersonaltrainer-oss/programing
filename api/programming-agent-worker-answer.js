...
export function createProgrammingAgentWorkerAnswerHandler({
  authorizeImpl = isProgrammingAgentWorkerAuthorized,
  broker = createProgrammingAgentVpsQueueBroker(),
  requestIdImpl = randomUUID,
} = {}) {
  return async function programmingAgentWorkerAnswerHandler(req, res) {
    ...
    try {
      ...
      const code = error instanceof ProgrammingAgentVpsQueueError ? error.code : 'answer_unavailable'
      return res.status(code === 'invalid_answer' ? 400 : code === 'ticket_unavailable' ? 409 : 503)
        .json({ error: code, requestId })
    }
  }
}

export default createProgrammingAgentWorkerAnswerHandler()
