export function evaluateFreshness(syncState, { now = new Date(), staleAfterMinutes = 90 } = {}) {
  const completed = syncState?.last_completed_at ? new Date(syncState.last_completed_at) : null
  ...
  return {
    ...
    stale: !healthy,
    lagMinutes,
    suppressNegativeActions: !healthy,
    status: healthy ? 'healthy' : syncState?.last_status === 'error' ? 'degraded' : 'stale',
  }
}
