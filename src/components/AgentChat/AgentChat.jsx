...
export default function AgentChat({
  ...
  return (
    <div className={`flex flex-col h-full min-h-0 ${coachBg.app}`}>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center py-12 animate-fade-in">
            ...
            <p className={`text-xs ${coachText.muted} max-w-xs leading-relaxed mb-8 font-medium`}>
            ...
            <div className="w-full max-w-sm space-y-2">
              <p className={`text-[9px] ${coachText.muted} font-bold uppercase tracking-widest mb-3`}>Accesos rápidos</p>
              <div className="grid grid-cols-2 gap-3">
                {QUICK_PROMPTS.map((p) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setInput(p)}
                    className={`w-full text-left px-4 py-2.5 rounded-xl ${coachBg.card} border ${coachBorder} hover:border-[#A729AD]/60 transition-all text-xs ${coachText.primary} font-medium`}
                  >
                    {p}
                  </button>
                ))}
          ...
          <div key={i}>
            ...
            {msg.role === 'assistant' && i === messages.length - 1 && !isGenerating && (
              <div className="mt-4 ml-10">
                {editingContent !== null ? (
                  <div className={`${coachBg.card} border ${coachBorder} rounded-2xl overflow-hidden animate-fade-in ${coachText.primary}`}>
                    <div className={`px-4 py-2.5 ${coachBg.app} border-b ${coachBorder} flex justify-between items-center`}>
                      <span className={`text-[10px] ${coachText.muted} font-bold uppercase tracking-widest`}>Editar Programación</span>
                      <button
                        type="button"
        ...
        {isGenerating && (
          <div className="flex items-center gap-3 ml-4 animate-fade-in">
            ...
            <div className={`flex items-center gap-2 px-5 py-3.5 ${coachBg.card} rounded-2xl rounded-tl-sm border ${coachBorder}`}>
      ...
      <div className={`px-6 py-5 border-t ${coachBorder} ${coachBg.sidebar} flex-shrink-0`}>
        {activeDayLabel && (
        ...
        <div className="flex gap-3 items-end">
          <div className={`flex-1 ${coachBg.card} border ${coachBorder} rounded-2xl overflow-hidden focus-within:border-[#A729AD]/50 focus-within:ring-1 focus-within:ring-[#A729AD]/20 transition-all`}>
            <textarea
              ref={textareaRef}
              value={input}
