...
export default function SessionPreview({ content, onConfirm, onEdit }) {
  ...
  return (
    <div className={`${coachBg.card} border ${coachBorder} rounded-2xl overflow-hidden`}>
          ...
          {(content.match(/https?:\/\/[^\s]+|[a-zA-Z0-9_-]{11}(?=\s|$)/g) || []).map((link, idx) => {
            ...
            return (
              <a
                ...
                className={`inline-flex items-center gap-2.5 px-4 py-3 rounded-2xl bg-[#6A1F6D]/20 border ${coachBorder} hover:border-[#A729AD]/50 transition-all group w-full`}
              >
                ...
                <div className="flex flex-col">
                  <span className={`text-[10px] font-bold ${coachText.primary} uppercase tracking-widest`}>Vídeo de la técnica</span>
                  <span className={`text-[9px] ${coachText.muted} font-bold truncate max-w-[200px]`}>{link}</span>
                </div>
      ...
      <div className={`px-5 py-5 ${coachBg.card} border-t ${coachBorder} space-y-4`}>
        <div>
          <p className={`text-[9px] ${coachText.muted} mb-2.5 font-bold uppercase tracking-widest`}>Clases incluidas</p>
