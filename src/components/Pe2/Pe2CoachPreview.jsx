...
function sessionTitle(session) {
  return session.title || session.class_type?.label || 'Sesión sin título'
}

function prescription(item) {
  return item.prescription || item.notes || item.text || ''
}

function CoachSession({ session }) {
  const blocks = [...(session.blocks || [])].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0))
  ...
  return (
    <article className="rounded-xl border p-4" style={{ backgroundColor: '#FFFFFF', borderColor: evoBrand.border }}>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.18em]" style={{ color: evoBrand.accent }}>
            Día {session.weekday ?? '—'} · {session.class_type?.label || 'Clase EVO'}
          </p>
          <h3 className="mt-1 text-lg font-bold" style={{ color: evoBrand.text }}>{sessionTitle(session)}</h3>
        </div>
      ...
      <div className="mt-4 grid gap-3">
        {blocks.map((block) => (
          <div key={block.id || block.sort_order} className="rounded-lg px-3 py-2" style={{ backgroundColor: '#FAF7FC' }}>
            <p className="text-xs font-bold uppercase tracking-wide" style={{ color: evoBrand.text }}>
              {block.title || block.block_type || 'Bloque'}
            </p>
            {(block.items || []).map((item) => (
              <p key={item.id || item.sort_order} className="mt-1 text-sm" style={{ color: evoBrand.muted }}>
                {item.exercise_name || item.name || 'Ejercicio'}{prescription(item) ? ` · ${prescription(item)}` : ''}
              </p>
            ))}
          </div>
        ))}
